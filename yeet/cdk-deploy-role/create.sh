...
if [ "$#" -lt 4 ]; then
    echo "usage: $0 <account> <region> <external-id> <alias>"
    exit 1
fi
...
role_name="tinstafl-deploy-$alias"
...
attach_policy "./policy/cloudformation.json" "$role_name" "$account" "$region" "$alias"
attach_policy "./policy/kms.json" "$role_name" "$account" "$region" "$alias"
attach_policy "./policy/s3.json" "$role_name" "$account" "$region" "$(echo "$alias" | tr '[:upper:]' '[:lower:]')"
attach_policy "./policy/ssm.json" "$role_name" "$account" "$region" "$alias"
attach_policy "./policy/sts.json" "$role_name" "$account" "$region" "$alias"
